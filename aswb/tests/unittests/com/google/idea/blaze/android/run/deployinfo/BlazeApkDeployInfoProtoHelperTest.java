...
public class BlazeApkDeployInfoProtoHelperTest extends BlazeTestCase {
  ...
  private ParsedManifestService mockParsedManifestService =
  ...
  public void readDeployInfoForNormalBuild_onlyMainManifest() throws Exception {
    ...
    when(mockAndroidDeployInfoReader.getDeployInfo(buildResultHelper, null))
    ...
    BlazeAndroidDeployInfo deployInfo =
        helper.readDeployInfoForNormalBuild(context, buildResultHelper);

    // verify
    assertThat(deployInfo.getApksToDeploy()).containsExactly(mainApk);
    assertThat(deployInfo.getManifestFiles()).containsExactly(mainManifestFile);
    ...
    verify(mockParsedManifestService, times(1))
        .invalidateCachedManifests(ImmutableList.of(mainManifestFile));
  }

  @Test
  public void readDeployInfoForNormalBuild_withTestManifest() throws Exception {
    // setup
    AndroidDeployInfo deployInfoProto =
        AndroidDeployInfo.newBuilder()
            .setMergedManifest(makeArtifact("path/to/manifest"))
            .addAdditionalMergedManifests(makeArtifact("path/to/test/manifest"))
            .addApksToDeploy(makeArtifact("path/to/apk"))
    ...
    ParsedManifest parsedMainManifest = new ParsedManifest("main", null, null);
    ParsedManifest parsedTestManifest = new ParsedManifest("test", null, null);
    ...
    // verify
    assertThat(deployInfo.getApksToDeploy()).containsExactly(mainApk, testApk);
    assertThat(deployInfo.getManifestFiles()).containsExactly(mainManifest, testManifest);
    assertThat(deployInfo.getMergedManifest()).isEqualTo(parsedMainManifest);
    assertThat(deployInfo.getTestTargetMergedManifest()).isEqualTo(parsedTestManifest);
    verify(mockParsedManifestService, times(1))
        .invalidateCachedManifests(ImmutableList.of(mainManifest, testManifest));
  }
  ...
  public void readDeployInfoForInstrumentationTest() throws Exception {
    ...
    // verify
    assertThat(deployInfo.getApksToDeploy()).containsExactly(instrumentorApk, testApk);
    assertThat(deployInfo.getManifestFiles()).containsExactly(instrumentorManifest, testManifest);
    assertThat(deployInfo.getMergedManifest()).isEqualTo(parsedInstrumentorManifest);
    assertThat(deployInfo.getTestTargetMergedManifest()).isEqualTo(parsedTestManifest);
    verify(mockParsedManifestService, times(1))
        .invalidateCachedManifests(ImmutableList.of(instrumentorManifest, testManifest));
  }
