...
def collect_android_ide_info(target, ctx, semantics, ide_info, ide_info_file, output_groups):
    ...
    if (hasattr(ctx.rule.attr, "resource_files")):
        for artifact_path_fragments, res_files in get_res_artifacts(ctx.rule.attr.resource_files).items():
            ...
            aar_file_name = target.label.name.replace("/", "-") + root.root_execution_path_fragment.replace("/", "-") + "-" + root.relative_path.replace("/", "-")
...
def make_intellij_info_aspect(aspect_impl, semantics):
    ...
    attrs = {
        ...
        "_create_aar": attr.label(
            default = Label("//aspect:create_aar"),
            cfg = "host",
            executable = True,
            allow_files = True,
        ),
        ...
        #    default = Label("//third_party/java/android/android_sdk_linux:static_aapt2_tool"),
        #),
    }
